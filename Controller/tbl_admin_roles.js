...
const addAdminRole = (req, res) => {
    try {
        ...
        connection.query(sqlQuery, data, (error, result) => {
            if (error) {
                console.log("Error:", error.sqlMessage);
                return res.status(500).json({ error: "Role addition failed" });
            }
    ...
    try {
        ...
        connection.query(sqlQuery, [data, id], (error, result) => {
            ...
            if (result.affectedRows > 0) {
                ...
                return res.json("roleid not found in database");
            }
        });
...
const getRoleId = (req, res) => {
    try {
        ...
        connection.query(sqlQuery, [rolename], (error, result) => {
            if (error) {
                console.log("Error:", error.sqlMessage);
                return res.status(500).json({ error: "Failed to fetch" });
            }
            ...
            if (result.length > 0) {
                return res.json({ roleid: result[0].roleid });
            } else {
