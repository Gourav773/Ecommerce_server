...
const updateSubCategory = (req, res) => {
    ...
    if (req.body.Pcategoryid) {
        updates.push("Pcategoryid = ?");
        params.push(req.body.Pcategoryid);
    }

    if (req.body.Subcategoryid) {
        updates.push("Subcategoryid = ?");
        params.push(req.body.Subcategoryid);
    }

    if (req.body.Subcategoryname) {
        updates.push("Subcategoryname = ?");
        params.push(req.body.Subcategoryname);
    }

    if (req.file) {
        updates.push("photo = ?");
        params.push(req.file.location || req.file.path);
    }
...
// BY NAME
const viewSubCategoryByName = (req, res) => {
...
// BY CATEGORY ID
const viewSubCategoryByPcategoryid = (req, res) => {
    const id = req.params.Pcategoryid;

    if (!id) {
        return res.status(400).json({ error: "Pcategoryid required" });
    }

    const sqlQuery = "SELECT * FROM tbl_admin_subcategory WHERE Pcategoryid = ?";
...
module.exports = {
    ...
    viewSubCategory,
    updateSubCategory,
    viewSubCategoryByName,
    viewSubCategoryByPcategoryid
};