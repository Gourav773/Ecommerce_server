...
// ADD RETAILER
const addRetailer = (req, res) => {
    const sqlCheckEmail = "SELECT * FROM tbl_retailer_register WHERE email=?";

    connection.query(sqlCheckEmail, [req.body.email], (err, data) => {
        ...
        bcrypt.hash(req.body.password, 10, (err, hash) => {
            ...
            connection.query("INSERT INTO tbl_retailer_register SET ?", userData, (error, result) => {
                if (error) {
...
// GET BY REGNO
const getRetailerByRegNo = (req, res) => {
    const sqlQuery = "SELECT * FROM tbl_retailer_register WHERE regno=?";

    connection.query(sqlQuery, [req.params.regno], (error, result) => {
...
// UPDATE
const updateRetailer = (req, res) => {
    const sqlQuery = "UPDATE tbl_retailer_register SET ? WHERE regno=?";

    connection.query(sqlQuery, [req.body, req.params.regno], (error, result) => {
        if (error) return res.status(500).json({ error: error.sqlMessage });
...
// STATUS UPDATE
const updateStatus = (req, res) => {
    const sqlQuery = "UPDATE tbl_retailer_register SET status=? WHERE regno=?";

    connection.query(sqlQuery, [req.query.status, req.query.regno], (err, result) => {
        if (err) return res.json("Error");

        return res.json(result.affectedRows ? "Updated" : "Not found");
    });
};
...
const updatePassword = (req, res) => {
    const sqlQuery = "UPDATE tbl_retailer_register SET password=? WHERE regno=?";

    connection.query(sqlQuery, [req.body.password, req.params.regno], (error, result) => {
        if (error) return res.status(500).json({ error: error.sqlMessage });
...
const updateDocuments = (req, res) => {
    const sqlQuery = "UPDATE tbl_retailer_register SET document_reg_no=?, docpan=?, docshop=? WHERE regno=?";

    connection.query(
        sqlQuery,
        [req.body.document_reg_no, req.body.docpan, req.body.docshop, req.params.regno],
        (error, result) => {
            if (error) return res.status(500).json({ error: error.sqlMessage });
...
const viewAllShops = (req, res) => {
    connection.query("SELECT * FROM tbl_retailer_register", (error, result) => {
        if (error) return res.status(500).json({ error: error.sqlMessage });
