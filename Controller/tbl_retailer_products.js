...
// ADD PRODUCT
const addRetailerProducts = (req, res) => {
    try {
        let sqlQuery = "INSERT INTO tbl_retailer_products SET ?";
        let data = {
            pid: req.body.pid,
...
// GET PRODUCTS BY REGNO
const getRetailerProduct = (req, res) => {
    try {
        const regno = req.params.regno;
        console.log("regno", regno);
...
const updateRetailerProductsPrice = (req, res) => {
    try {
        ...
        connection.query(sqlQuery, [newPrice, pid], (err, result) => {
            if (err) {
                console.log("Error:", err.sqlMessage);
                return res.status(500).json({ error: 'server error' });
            }
...
const updateRetailerProductsDiscount = (req, res) => {
    try {
        ...
        connection.query(sqlQuery, [discount, pid], (err, result) => {
            if (err) {
                console.log("Error:", err.sqlMessage);
                return res.status(500).json({ error: 'server error' });
            }
...
const updateRetailerProductsQuantity = (req, res) => {
    try {
        ...
        connection.query(sqlQuery, [quantity, pid], (err, result) => {
            if (err) {
                console.log("Error:", err.sqlMessage);
                return res.status(500).json({ error: 'server error' });
            }
...
const getAdminAllProducts = (req, res) => {
    try {
        let sqlQuery = `
            SELECT
                p.*,
...
module.exports = {
    addRetailerProducts,
    getRetailerProduct,
    updateRetailerProductsPrice,
    updateRetailerProductsDiscount,
    updateRetailerProductsQuantity,
    getAdminAllProducts
};